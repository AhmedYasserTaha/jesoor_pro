...
class _OtpVerificationDialogState extends State<OtpVerificationDialog> {
  ...
  void _clearError(BuildContext context) {
    ...
    if (widget.isLogin) {
      ...
      if (loginCubit.state.loginVerifyOtpStatus == LoginStatus.error) {
        loginCubit.clearLoginVerifyOtpError();
      }
    } else {
      final signupCubit = context.read<SignupCubit>();
      if (signupCubit.state.verifyOtpStatus == SignupStatus.error) {
        signupCubit.clearVerifyOtpError();
      }
    }
  }
    ...
    return widget.isLogin
        ? BlocBuilder<LoginCubit, LoginState>(
            builder: (context, state) {
              final isLoading =
                  state.loginVerifyOtpStatus == LoginStatus.loading;
              final hasError = state.loginVerifyOtpStatus == LoginStatus.error;
              final errorMessage = state.errorMessage ?? Strings.otpIncorrect;
              return _buildDialog(context, isLoading, hasError, errorMessage);
            },
          )
        : BlocBuilder<SignupCubit, SignupState>(
            builder: (context, state) {
              final isLoading = state.verifyOtpStatus == SignupStatus.loading;
              final hasError = state.verifyOtpStatus == SignupStatus.error;
              final errorMessage = state.errorMessage ?? Strings.otpIncorrect;
              return _buildDialog(context, isLoading, hasError, errorMessage);
            },
          );
